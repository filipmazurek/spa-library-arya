...
class Property7Threshold(BaseProperty, abc.ABC):
...
class Property7(Property7Threshold, GenericProperty):
    """Property comparison model to compare a single execution property against a threshold.
    Table 1, item 7 in the paper https://doi.org/10.1145/3613424.3623785."""
    ...
    def start_point_estimate(data: pd.DataFrame, proportion: float) -> float:
        ...
        # Get a list of the request 2 latencies
        data_list = []
        for x in pd.unique(pd.Series(data['run'])):

            # Get the request 2 start and end cycle numbers and calculate latency
            event2_start = data.query('run == ' + str(x) + ' and tag == "request 2 start"')['value'].tolist()[0]
            event2_end = data.query('run == ' + str(x) + ' and tag == "request 2 end"')['value'].tolist()[0]
            data_list.append(event2_end - event2_start)
    ...
    def extract_value(self, data: pd.DataFrame) -> Tuple[Tuple[_T_NUMBER, _T_NUMBER], List[_T_NUMBER]]:
        ...
        event1_start = data.query('run == ' + str(run_number) + ' and tag == "request 1 start"')['value'].tolist()[0]
        event1_end = data.query('run == ' + str(run_number) + ' and tag == "request 1 end"')['value'].tolist()[0]
        event2_start = data.query('run == ' + str(run_number) + ' and tag == "request 2 start"')['value'].tolist()[0]
        event2_end = data.query('run == ' + str(run_number) + ' and tag == "request 2 end"')['value'].tolist()[0]
        value = [event1_end-event1_start, event2_end-event2_start]
